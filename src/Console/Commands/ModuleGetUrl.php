    ...
    public function handle()
    {
        ...
        if($url) {
            ...
            if($filename) {
                ...
                $path = base_path('modules').DIRECTORY_SEPARATOR."__";
                if(!is_dir($path)) mkdir($path);

                $dir = scandir($path);
                $foldername = $path.DIRECTORY_SEPARATOR.$dir[2];
                $jsonBody = file_get_contents($foldername.DIRECTORY_SEPARATOR."module.json");
                $module = json_decode($jsonBody);

                if(file_exists($path)) {
                    passthru("mv ".$foldername." ".base_path('modules').DIRECTORY_SEPARATOR.$module->name);
                    //exec("mv ".$foldername." ".base_path('modules').DIRECTORY_SEPARATOR.$module->name);
                    rmdir($path);
